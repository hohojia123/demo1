      ...
      <el-row>
        <el-menu :default-active="activeIndex" class="el-menu-demo" :router="true" mode="horizontal"
          ...
          <el-submenu class="hidden-xs-only" index="4" v-if="this.$store.state.token!==''" :router="true">
            <template slot="title">
              <img v-if="this.$store.state.avatar" :src="this.$store.state.avatar" style="width: 28px; height: 28px; border-radius: 50%; vertical-align: middle; margin-right: 5px; object-fit: cover;">
              <i v-else class="el-icon-user-solid" style="font-size: 28px; vertical-align: middle; margin-right: 5px;"></i>
            ...
            <el-menu-item route="/admins/userManage" index="4-4" v-if="this.$store.state.roles.toUpperCase().indexOf('ADMIN') > -1">
              &nbsp;&nbsp;&nbsp;
...
//        console.log(key, keyPath);
      ...
      logout() {  //退出登录
        user.logout().then(res => {
          ...
          sendMail() {  //发送验证码
            if (this.form.registerMail.length <= 0) {
              this.$message({
                message: '邮箱不能为空',
                type: 'error'
              });
           ...
           userRegister() {//用户注册
             ...
             user.register(this.form.registerName, this.form.registerPwd, this.form.registerMail, this.form.registerMailCode).then(res => {
               this.$message({
                 message: '注册成功',
                 type: 'success'
               })
