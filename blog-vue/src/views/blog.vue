      ...
      <div style="text-align: center">
        <p>
          ...
          <span class="el-icon-user-solid hidden-xs-only" style="margin-left: 150px">
            <img v-if="userAvatar" :src="userAvatar" style="width: 30px; height: 30px; border-radius: 50%; vertical-align: middle; margin-right: 5px; object-fit: cover;">
            {{userName}}
          </span>
        </p>
        <p>
      ...
      <div id="discuss" class="hidden-xs-only">
        ...
        <div v-for="discuss in discussList" id="discussList">
          <p style="margin: -5px " @mouseenter="pEnter()" @mouseleave="pLeave()">
            <img v-if="discuss.user.avatar" :src="discuss.user.avatar" style="width: 30px; height: 30px; border-radius: 50%; vertical-align: middle; margin-right: 5px; object-fit: cover;">
            <i v-else class="el-icon-user-solid" style="font-size: 30px; vertical-align: middle; margin-right: 5px;"></i>
            <span style="cursor: pointer;">{{discuss.user.name}}&nbsp;&nbsp;:</span>
            ...
            <span v-if="(discuss.user.name==getStoreName()||(getStoreRoles().indexOf('ADMIN') > -1))&&replyFlag"
                  style="margin-left: 5%; cursor: pointer; color: #409EFF;"
                  @click="deleteDiscuss(discuss.id)">删除
            </span>
            <span v-if="getStoreName()!=''&&replyFlag"
                  style="margin-left: 1%; cursor: pointer; color: #409EFF;"
                  @click="sendReply(discuss.id,null)">回复
            </span>
          ...
          <p v-if="!(typeof(discuss.replyList) == 'undefined') && discuss.replyList.length>0"
            ...
            <img v-if="reply.user.avatar" :src="reply.user.avatar" style="width: 26px; height: 26px; border-radius: 50%; vertical-align: middle; margin-right: 5px; object-fit: cover;">
            <i v-else class="el-icon-user-solid" style="font-size: 26px; vertical-align: middle; margin-right: 5px;"></i>
            <span style="cursor: pointer;">{{reply.user.name}}&nbsp;&nbsp;:</span>

            <span v-if="reply.reply !== null">回复:</span>
            <span v-if="!(typeof(reply.reply) == 'undefined') && reply.reply !== null">
            ...
            <span v-if="(reply.user.name==getStoreName()||(getStoreRoles().indexOf('ADMIN') > -1))&&replyFlag"
                  style="margin-left: 5%; cursor: pointer; color: #409EFF;"
                  @click="deleteReply(reply.id)">删除
            </span>
            <span v-if="getStoreName()!=''&&replyFlag"
                  style="margin-left: 1%; cursor: pointer; color: #409EFF;"
                  @click="sendReply(discuss.id,reply.id)">回复
            </span>
          </p>
        </div>
